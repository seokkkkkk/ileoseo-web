...
export default function Slider({ images }: { images: string[] }) {
    ...
    return (
        <div className="relative">
            ...
            <button
                onClick={prev}
                className="absolute top-1/2 left-3 -translate-y-1/2 bg-white/80 rounded-full px-4 py-2 shadow hover:bg-white cursor-pointer"
                aria-label="이전"
            >
                ◀
            </button>
            <button
                onClick={next}
                className="absolute top-1/2 right-3 -translate-y-1/2 bg-white/80 rounded-full px-4 py-2 shadow hover:bg-white cursor-pointer"
                aria-label="다음"
            >
                ▶
            </button>
